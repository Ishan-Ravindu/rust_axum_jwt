...
pub async fn guard<T>(
    ...
    is_valid(&token)?; // Validating token after getting from the database to obfuscate that the token is wrong. Feel free to move up if you are not worried about that.
